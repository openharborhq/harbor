...
- expiresAt: only when the document itself stops being valid on a date — a passport expiry, a policy end, a registration that lapses. NOT a payment due date and NOT a deadline for doing something; those belong in obligations below. Otherwise null.
...
- obligations: anything this document says the reader still has to DO, at most three, and only when the document itself says so. kind is one of pay (a bill, invoice or instalment), file (return, submit or send something back), renew (a registration or membership that must be renewed), fetch (the document itself is only a notification and the real paperwork has to be collected from a portal or a branch), review (a decision or objection with a deadline). title is a short imperative naming the sender — "Pay Stadtwerke invoice 4471", "Return the signed Kita registration". dueOn is the stated deadline as ISO YYYY-MM-DD, or null if the document gives none. amountCents is the amount in minor units (€248.10 is 24810) for a payment, else null. currency is one of EUR, USD, GBP or CHF, read from what the document actually shows — a € sign, "Euro" or "EUR"; a $ sign, "Dollars" or a US address; a £ sign; "CHF" or "Franken". Never assume one from the reader's language: if the document shows no currency anywhere, set currency to null and the reader will choose. When a document shows several amounts — subtotals, line items, tax, a prior balance, an amount already paid — take the one the reader must actually pay: the final total due, usually the largest and usually labelled total, balance due, amount due, zu zahlen or fällig. Never sum them yourself, and never take a line item. An invoice that is already marked paid has no obligation. Empty list when the document asks nothing of anyone — a certificate, a statement, a record.
...
export function documentBlock(input: SuggestInput): string {
  const meta = [
    ...
    .filter(Boolean)
    .join("\n");
  const heading = input.textTruncated
    ? "EXTRACTED TEXT — a sample from every page; the document is longer than what is shown and each […] marks a cut"
